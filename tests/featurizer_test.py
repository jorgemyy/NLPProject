...
from package.graph import Node
...
def test_get_features_from_graph(obama_graphs, embedding_model):
    ...
    first_word_head = test_graph.nodes[0].head
    ...
    num_node_features = num_normalized_features + num_upos + num_xpos + embedding_model.vector_size
    assert features.shape == (num_words,num_node_features)


def test_one_hot_encoding_on_gettysburg(gettys_graphs):
...
def test_featurizer_on_gettysburg_by_sentence(gettys_graphs,embedding_model,ud_gettys_doc):
